...
(function() {
    ...
    function replaceFriendSessionText(username, title) {
        ...
        friendElements.forEach(friend => {
            ...
            if (nameElement && statusElement && nameElement.textContent === username) {
...
(function() {
    'use strict';
    async function fetchInvitations(action) {
        const selfUserId = prompt("Enter your own User ID for invitations:");
        const url = `https://www.kogama.com/user/${selfUserId}/invitations/`;
        GM_xmlhttpRequest({
            method: 'GET',
            url: url,
            onload: function(response) {
                ...
                invitations.forEach(invitation => {
                    const gameId = invitation.game_id;
                    const requestUrl = `https://www.kogama.com/game/${gameId}/member/${selfUserId}/`;
                    ...
                    if (action === 'accept') {
                        GM_xmlhttpRequest({
                            ...
                            url: requestUrl,
                            onload: function() {
                                showNotification('accept', `Accepted project invitation for game ID ${gameId}`);
                            }
                        });
                        ...
                        GM_xmlhttpRequest({
                            ...
                            url: requestUrl,
                            onload: function() {
                                showNotification('reject', `Rejected project invitation for game ID ${gameId}`);
                            }
                        });
                    }
                });
    ...
    function showNotification(type, message) {
        const notificationTypeStyles = {
            'accept': { backgroundColor: '#50fa7b', borderColor: '#50fa7b' },
            'reject': { backgroundColor: '#ff5555', borderColor: '#ff5555' }
        };
        const notification = $('<div>')
            .text(message)
            .css({
                position: 'fixed',
                bottom: type === 'accept' ? '120px' : '80px',
                left: '50%',
    ...
    function createMenu() {
        ...
        function createMenuButton(label, action) {
            return $('<button>', { text: label, class: `btn-${Math.random().toString(36).substring(2, 8)}` })
                .css({
                    padding: '10px 20px',
                    fontSize: '14px',
                    color: '#ff79c6',
                ...
                .hover(
                    function() { $(this).css('backgroundColor', '#6272a4'); },
                    function() { $(this).css('backgroundColor', '#44475a'); }
                )
                .on('click', action);
        }
        const leaveProjectsAction = () => fetchInvitations('leave');
        const acceptProjectsAction = () => fetchInvitations('accept');
        const rejectProjectsAction = () => fetchInvitations('reject');
        menuContainer.append(
            createMenuButton('Leave Projects', leaveProjectsAction),
            createMenuButton('Reject Projects', rejectProjectsAction),
            createMenuButton('Accept Projects', acceptProjectsAction)
        );
    }
    createMenu();
})()
