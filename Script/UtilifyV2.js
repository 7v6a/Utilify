...
(function() {     // Custom Website Gradient - command exec.[ utilify.gradient() ]
    window.utilify = window.utilify || {};
    window.utilify.gradient = function() {
        if (document.getElementById('utilify-gradient-editor')) {
            console.log('Editor already exists');
            return;
        }

        const styleId = 'utilify-gradient-styles';
        if (!document.getElementById(styleId)) {
            const style = document.createElement('style');
            style.id = styleId;
            style.textContent = `
                #utilify-gradient-editor {
                    position: fixed;
                    top: 50%;
                    left: 50%;
                ...
                #utilify-gradient-editor.hidden {
                    opacity: 0;
                ...
                .utilify-close-btn:hover {
                    background: #ff5555;
                }

                .utilify-close-btn:active {
                    background: #ff3333;
        ...
        editor.id = 'utilify-gradient-editor';
        editor.innerHTML = `
            <div class="utilify-title">
                <span>Gradient Editor</span>
                <button class="utilify-close-btn" id="utilify-close-btn">✕</button>
            </div>
            <div class="utilify-input-group">
                <div class="utilify-label">Colors</div>
                <input type="color" id="utilify-color1" value="#667eea">
                <span class="utilify-color-preview" id="utilify-preview1" style="background:#667eea"></span>
                <input type="color" id="utilify-color2" value="#764ba2">
                <span class="utilify-color-preview" id="utilify-preview2" style="background:#764ba2"></span>
            </div>
            <div class="utilify-input-group">
                <div class="utilify-label">Angle: <span id="utilify-angle-value">135</span>°</div>
                <input type="range" id="utilify-angle" min="0" max="360" value="135">
            </div>
            <div class="utilify-input-group">
                <div class="utilify-label">Custom Gradient</div>
                <input type="text" id="utilify-custom-gradient" placeholder="linear-gradient(135deg, #667eea, #764ba2)">
            </div>
            <div style="margin-top:15px;">
                <button class="utilify-btn" id="utilify-copy-btn">Copy Gradient</button>
                <button class="utilify-btn" id="utilify-clear-btn">Clear Gradient</button>
            </div>
        `;
        document.body.appendChild(editor);
        const savedGradient = localStorage.getItem('utilifyGradient') || 'linear-gradient(135deg, #667eea, #764ba2)';
        document.body.style.background = savedGradient;
        const parseGradient = (grad) => {
            const match = grad.match(/linear-gradient\((\d+)deg,\s*(#[0-9a-f]+),\s*(#[0-9a-f]+)/i);
            if (match) {
                return {
                    angle: match[1],
                    color1: match[2],
        ...
        const saved = parseGradient(savedGradient);
        if (saved) {
            document.getElementById('utilify-color1').value = saved.color1;
            document.getElementById('utilify-color2').value = saved.color2;
            document.getElementById('utilify-angle').value = saved.angle;
            document.getElementById('utilify-angle-value').textContent = saved.angle;
            document.getElementById('utilify-preview1').style.background = saved.color1;
            document.getElementById('utilify-preview2').style.background = saved.color2;
            document.getElementById('utilify-custom-gradient').value = savedGradient;
        }
        const updateGradient = () => {
            const color1 = document.getElementById('utilify-color1').value;
            const color2 = document.getElementById('utilify-color2').value;
            const angle = document.getElementById('utilify-angle').value;
            const gradient = `linear-gradient(${angle}deg, ${color1}, ${color2})`;

            document.body.style.background = gradient;
            localStorage.setItem('utilifyGradient', gradient);

            document.getElementById('utilify-preview1').style.background = color1;
            document.getElementById('utilify-preview2').style.background = color2;
            document.getElementById('utilify-angle-value').textContent = angle;
            document.getElementById('utilify-custom-gradient').value = gradient;
        };
        const handleCustomGradient = (e) => {
            const value = e.target.value;
            if (value.includes('linear-gradient')) {
                document.body.style.background = value;
                localStorage.setItem('utilifyGradient', value);

                const parsed = parseGradient(value);
                if (parsed) {
                    document.getElementById('utilify-color1').value = parsed.color1;
                    document.getElementById('utilify-color2').value = parsed.color2;
                    document.getElementById('utilify-angle').value = parsed.angle;
                    document.getElementById('utilify-angle-value').textContent = parsed.angle;
                    document.getElementById('utilify-preview1').style.background = parsed.color1;
                    document.getElementById('utilify-preview2').style.background = parsed.color2;
                }
            }
        };
        document.getElementById('utilify-color1').addEventListener('input', updateGradient);
        document.getElementById('utilify-color2').addEventListener('input', updateGradient);
        document.getElementById('utilify-angle').addEventListener('input', updateGradient);
        document.getElementById('utilify-custom-gradient').addEventListener('input', handleCustomGradient);

        document.getElementById('utilify-copy-btn').addEventListener('click', () => {
            const gradient = localStorage.getItem('utilifyGradient') || document.body.style.background;
            navigator.clipboard.writeText(gradient).then(() => {
                alert('Gradient copied to clipboard!');
            });
        });

        document.getElementById('utilify-clear-btn').addEventListener('click', () => {
            localStorage.removeItem('utilifyGradient');
            document.body.style.background = '';
            document.getElementById('utilify-custom-gradient').value = '';
        });
        document.getElementById('utilify-close-btn').addEventListener('click', () => {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const saved = localStorage.getItem('utilifyGradient');
        if (saved) {
