...
(function () {
  "use strict";
  const regexPattern =
    /(https?:\/\/(?:www\.)?(?:kogama\.com\.br|friends\.kogama\.com|www\.kogama\.com)\/profile\/\d+\/|https?:\/\/(?:www\.)?(?:kogama\.com\.br|friends\.kogama\.com|www\.kogama\.com)\/profile\/[A-Z]+UID\/)/g;
...
(function () {
  ...
  function logUserId() {
    ...
    for (let script of scripts) {
      if (script.textContent.includes("options.bootstrap")) {
        try {
          const bootstrapRegex = /options\.bootstrap\s*=\s*({.*?});/s;
          const match = bootstrapRegex.exec(script.textContent);

          if (match && match[1]) {
            const options = eval(`(${match[1]})`);
            if (options.object && options.object.id) {
              const userId = options.object.id;
              fetchFriendChat(userId);
            }
          }
          break;
        } catch (error) {
        ...
        if (data.data && Array.isArray(data.data)) {
          data.data.forEach((friend) => {
            ...
            if (location !== "N/A") {
              ...
              const projectMatch = location.match(
                /\/build\/\d+\/project\/(\d+)\//
              );
              if (projectMatch) {
  ...
  function fetchGameTitle(GID, username) {
    ...
    fetch(url)
      ...
      .then((html) => {
        ...
        const gameTitle = title.split(" - KoGaMa")[0].trim();
        gameTitles[username] = gameTitle;
        replaceFriendSessionText(username, gameTitle);
      })
      .catch((error) => {
  ...
  const projectNames = {};
  function fetchProjectName(POID, username) {
    const url = `https://www.kogama.com/game/${POID}/member`;
    ...
    fetch(url)
      ...
      .then((data) => {
        if (data.data && Array.isArray(data.data) && data.data.length > 0) {
          ...
          projectNames[username] = projectName;
          replaceFriendSessionText(username, projectName);
        }
      })
      .catch((error) => {
        console.error(
  ...
  function replaceFriendSessionText(username, title) {
    const friendElements = document.querySelectorAll("._1taAL");
    friendElements.forEach((friend) => {
      const nameElement = friend.querySelector("._3zDi-");
      const statusElement = friend.querySelector("._40qZj");
...
(function () {
  ...
  init();
})(); // project manipulation (leave ; reject ; accept)
(function () {
  "use strict";
  if (window.location.href !== "https://www.kogama.com/build/") return;

  let userId = localStorage.getItem("kogama_user_id") || "";

  async function fetchProjects() {
    let page = 1;
    let projects = [];
    while (true) {
      let url = `https://www.kogama.com/game/?page=${page}&count=12&filter=member`;
  ...
  async function modifyProjects(action) {
    if (!userId) return;
    let projects = await fetchProjects();
    projects.forEach((project) => {
      let url = `https://www.kogama.com/game/${project.id}/member/${userId}/`;
      let method = action === "accept" ? "PUT" : "DELETE";
      GM_xmlhttpRequest({
        method: method,
        url: url,
        onload: () =>
          showNotification(
            ...
            `${action}ed project: ${project.name}`
          ),
      });
    });
  }
  ...
  function createMenu() {
    ...
    function createInput() {
      ...
      input.addEventListener("change", () => {
        userId = input.value;
        localStorage.setItem("kogama_user_id", userId);
      });
      return input;
    }
    ...
    menu.appendChild(
      createButton("Remove Projects", () => modifyProjects("remove"))
    );
    menu.appendChild(
      createButton("Reject Projects", () => modifyProjects("reject"))
    );
    menu.appendChild(
      createButton("Accept Projects", () => modifyProjects("accept"))
    );
          ...
          if (data && data.error) {
            ...
            if (data.error.__all__) {
              ...
              if (detailedError.includes("banned")) {
                ...
                const remainingTimeMatch = detailedError.match(
                  /You'll remain banned for another ([\d\D]+)\./
                );
                const remainingTime = remainingTimeMatch
                  ? remainingTimeMatch[1].trim()
                  : "";
                errorDisplay.innerHTML = `
                                ...
                                `;
                errorDisplay.title = detailedError
                  .split("due to:")[1]
                  .split("\n")[0]
                  .trim();
              } else {
...
(function () {
  ...
  function formatText(text) {
    ...
    formattedText = formattedText.replace(patterns.link, (match, text, url) => {
      ...
      return href
        ? `<a class="markdown-link" href="${href}" target="_blank" rel="noopener noreferrer">${text}</a>`
        : text;
    });
    formattedText = formattedText.replace(
      patterns.list,
...
(function () {
  ...
  const button = $('<button id="scrapeButton">Download Account Log</button>')
    .css({
      position: "fixed",
      bottom: "20px",
      left: "50%",
  ...
  function showLoadingOverlay() {
    ...
    const message = $(
      '<div id="loadingMessage">Acquiring Account Credentials...</div>'
    ).css({
      color: "#fff",
      fontSize: "18px",
      marginTop: "20px",
    ...
    GM_xmlhttpRequest({
      ...
      onload: function (response) {
        try {
          ...
          if (userDataScript) {
            const bootstrapDataMatch = userDataScript.innerHTML.match(
              /options\.bootstrap\s*=\s*({[\s\S]*?});/
            );
  ...
  function fetchGameData(counter, games, purchases, accountDetails) {
    if (counter > 30) {
      generateSHA256Hash(JSON.stringify(accountDetails) + Date.now())
        .then((backupHash) => {
          const accountLog = {
...
(function () {
  ...
  var { label: objectIdLabel, input: objectIdInput } = createLabelWithInput(
    "ID:",
    "text"
  );
  menuDiv.appendChild(objectIdLabel);

  var { label: loopAmountLabel, input: loopAmountInput } = createLabelWithInput(
    "TIMES TO BUY:",
    "number"
  );
  ...
  function displayLastOnlineStatus() {
    ...
    for (let script of scripts) {
      if (script.innerText.includes("last_ping")) {
        let lastPingMatch = script.innerText.match(
          /"last_ping"\s*:\s*"([^"]+)"/
        );
    ...
    if (lastPing) {
      ...
      if (diffMins < 5) {
        ...
        // within 15 minutes - last offline $amount minutes ago
        statusText = `Last online ${diffMins} minute${
          diffMins > 1 ? "s" : ""
        } ago`;
        color = "#f8b38a";
      } else {
      ...
      let targetElement = targetContainer
        ? targetContainer.querySelector("span._20K92")
        : null;
...
(function () {
  ...
  .features-page button.close {
      ...
      top: 10px;
      right: 10px;
      background-color: rgba(162, 119, 255, 0.15);
      color: #a277ff;
  ...
  setTimeout(function () {
    var header =
      document.querySelector(
        "body#root-page-mobile > div#root:nth-of-type(1) > div > header.MuiPaper-root.MuiPaper-elevation.MuiPaper-elevation4.MuiAppBar-root.MuiAppBar-colorPrimary.MuiAppBar-positionStatic._1q4mD.css-x84yyj > div.MuiToolbar-root.MuiToolbar-gutters.MuiToolbar-dense._1sUGu.css-qizlhk > div._2mwlM > div._290sk:nth-of-type(3)"
      ) ||
      document.querySelector(
        "body#root-page-mobile > div#nav-bar > header.MuiPaper-root.MuiPaper-elevation.MuiPaper-elevation4.MuiAppBar-root.MuiAppBar-colorPrimary.MuiAppBar-positionStatic._1q4mD.css-x84yyj > div.MuiToolbar-root.MuiToolbar-gutters.MuiToolbar-dense._1sUGu.css-qizlhk > div._2mwlM > div._290sk"
      );
    ...
    document
      .querySelector(".features-page .close")
...
(function () {
  ...
  const contributorArray = {
    ...
    50117938: [
      "contributor",
      "https://cdn.discordapp.com/avatar-decoration-presets/a_d9ff5ff133ed9176895a4a2b5e58f1b8.png",
    ], // R
    668318153: [
      "contributor",
    ...
    17037147: [
      "",
      "https://cdn.discordapp.com/avatar-decoration-presets/a_48b8411feb1e80a69048fc65b3275b75.png",
    ], // A
  };
      ...
      const observer = new MutationObserver((mutations) => {
        ...
        if (profileStatsContainer) {
          ...
          if (badgeType === "contributor") {
            ...
            badge.title =
              "Official Utilify Contributor. Thank you for your help ♡";
          }
          badgesContainer.appendChild(badge);
          profileStatsContainer.parentNode.insertBefore(
            badgesContainer,
            profileStatsContainer.nextSibling
          );
...
(function () {
  ...
  function showUBHelper(event) {
    ...
    if (
      backgroundIndex !== -1 &&
      (backgroundIndex > filterIndex || filterIndex === -1)
    ) {
...
(function () {
  ...
  function addPasswordGeneratorUI() {
    ...
    generatorDiv
      .querySelector(".close-btn-l1337x")
    ...
    generatorDiv
      .querySelector("#lengthInput-l1337x")
      .addEventListener("input", (e) => {
        generatorDiv.querySelector("#lengthValue-l1337x").value =
          e.target.value;
        generateAndUpdatePassword();
      });

    generatorDiv
      .querySelector("#lengthValue-l1337x")
      .addEventListener("input", (e) => {
        generatorDiv.querySelector("#lengthInput-l1337x").value =
          e.target.value;
        generateAndUpdatePassword();
      });
    ...
    generatorDiv
      .querySelector("#copyButton-l1337x")
      .addEventListener("click", () => {
        const passwordText = generatorDiv.querySelector(
          "#generatedPassword-l1337x"
        ).textContent;
        if (passwordText) {
          navigator.clipboard
  ...
  function generateAndUpdatePassword() {
    const length = parseInt(
      document.querySelector("#lengthInput-l1337x").value,
      10
    );
    const includeNumbers = document.querySelector(
      "#numCheckbox-l1337x"
    ).checked;
    const includeSpecial = document.querySelector(
      "#specialCheckbox-l1337x"
    ).checked;
    const includeUpper = document.querySelector(
      "#upperCheckbox-l1337x"
    ).checked;
    const includeLower = document.querySelector(
      "#lowerCheckbox-l1337x"
    ).checked;
  ...
  function dragElement(elmnt) {
    ...
    if (document.querySelector(".draggable-header-l1337x")) {
      document.querySelector(".draggable-header-l1337x").onmousedown =
        dragMouseDown;
    } else {
...
(async () => {
  ...
  function addButton() {
    ...
    customButton.className =
      "MuiButtonBase-root MuiButton-root MuiButton-text CustomButton";
  ...
  async function processPageData() {
    ...
    const pageButtons = paginator.querySelectorAll(
  ...
  function convertEmoji(text, emojiArray) {
    for (let emoji in emojiArray) {
      ...
      text = text.replace(
        ...
        `<img src="${
          emojiArray[emoji]
        }" alt="${emoji}" title="${emoji.substring(
          1,
          emoji.length - 1
...
(function () {
  ...
  fetch(requestUrl)
    ...
    .then((data) => {
      ...
      const formattedDate = `${createdDate.getDate()} ${getMonthName(
        createdDate.getMonth()
      )} ${createdDate.getFullYear()}, ${createdDate.getHours()}:${padZero(
...
(function () {
  ...
  if (window.location.href.endsWith("/gradient")) {
    ...
    function updateGradient() {
      const startColor =
        startColorInput.querySelector("input[type=color]").value;
...

{
  ...
  setTimeout(() => {
    ...
    console.log(
      "%c" + "Report any suspicious users who advised you to open this.",
      ConsoleStyle.NORMAL
    );
    console.log(
      "%c" + "Want to help out? Check out -> github.com/ctaah/Utilify",
      ConsoleStyle.INVITE
    );
    console.log(
      "%c" + "Need support? Discord: 970332627221504081",
  ...
  async function fetchImageSource(gameId) {
    try {
      ...
      const script = Array.from(doc.querySelectorAll("script")).find((s) =>
        s.textContent.includes("options.bootstrap")
      );
        ...
        if (match && match[1]) {
          ...
          if (imageUrls) {
            const filteredImageUrl =
              imageUrls.large ||
              Object.values(imageUrls).find((url) =>
                url.endsWith("600x240.jpg")
              );
            if (filteredImageUrl) {
...
(function () {
  ...
  function createCategory(title, settings) {
    ...
    settings.forEach((setting) => {
      const settingControl = createCheckbox(
        setting.key,
        setting.callback,
        setting.label
      );
  ...
  function applyActivityBlockerSetting(isEnabled) {
    ...
    if (isEnabled) {
      XMLHttpRequest.prototype.open = function (
        ...
        if (
          method === "POST" &&
          /https:\/\/www\.kogama\.com\/user\/\d+\/pulse\//.test(url)
        ) {
          return;
        }
  ...
  function applyBadgeSettings() {
    ...
    GM_addStyle(`
                ._1EHSt._3qLso {
                    display: ${
                      disableHiddenBadges ? "none" : "block"
                    } !important;
                }
                ._1z4jM {
...
(function () {
  ...
  function containsTargetClasses(element) {
    return targetClassNames.some((className) =>
  ...
  const InsertBanner = async () => {
    try {
      ...
      if (match && typeof match == "object") {
        ...
        subtitleTextElement.style.textShadow = "2px 2px 4px rgba(0, 0, 0, 0.3)";
...
(function () {
  ...
  function parseMarkdown(text) {
    ...
    const processedLines = lines.map((line) => {
      ...
      line = line.replace(
        /`(.+?)`/g,
        '<code style="background-color: #524B49; padding: 2px; border-radius: 3px;">$1</code>'
      );
...
(function () {
  ...
  const isOnProfilePage = /^https:\/\/www\.kogama\.com\/profile\/\d+\/?$/.test(
    window.location.href
  );

  function isMe() {
    const scriptTags = [...document.head.getElementsByTagName("script")];
    const bootstrapScript = scriptTags.find((script) =>
      script.textContent.includes("options.bootstrap")
    );
    ...
    const bootstrapObjectMatch = bootstrapText.match(
      /options\.bootstrap\s*=\s*(\{.*?\});/s
    );
  ...
  async function fetchAndDisplayFeeds() {
    ...
    async function deleteFeed(feedId) {
      ...
      try {
        ...
        if (!response.ok) {
          console.error(
            `Failed to delete feed item ${feedId}: HTTP status ${response.status}`
          );
        } else {
          const feedItem = document.querySelector(
            `.feedItem[data-feed-id='${feedId}']`
          );
          if (feedItem) {
    ...
    function toggleComments(feedItem, feedId) {
      ...
      fetchComments(feedId).then(({ data: comments }) => {
        ...
        if (
          commentContainer &&
          commentContainer.classList.contains("commentContainer")
        ) {
        ...
        if (comments.length === 0) {
          const noCommentsInfo = createElementWithStyle(
            "div",
            noCommentsInfoStyle
          );
          ...
          const commentListElement = createElementWithStyle(
            "ul",
            commentListStyle
          );
          comments.forEach((comment) => {
            ...
            const avatarUrl = comment.images
              ? comment.images.medium
              : "https://www.kogstatic.com/placeholder/blockboy_medium_64x64.jpg";
            ...
            commentContent.innerHTML = `<strong>${
              comment.profile_username
            }</strong><br>${JSON.parse(comment._data).data}`;
            commentContent.style.marginRight = "40px";

            listItem.appendChild(avatar);
            listItem.appendChild(commentContent);

            const deleteCommentButton = createButton(
        ...
        feedItem.parentNode.insertBefore(
          commentContainer,
          feedItem.nextSibling
        );
    ...
    async function deleteComment(commentId) {
      ...
      try {
        ...
        if (!response.ok) {
          console.error(
            `Failed to delete comment ${commentId}: HTTP status ${response.status}`
          );
        } else {
    ...
    function createFeedItem(item) {
      ...
      const avatarUrl = item.avatar_images
        ? item.avatar_images.medium
        : "https://www.kogama.com/placeholder/blockboy_medium_64x64.jpg";
      const avatar = document.createElement("img");
      avatar.src = avatarUrl;
      ...
      dateElement.textContent = new Date(item.created).toLocaleDateString(
        "en-GB",
        {
          weekday: "short",
          year: "numeric",
      ...
      message.textContent = item._data
        ? JSON.parse(item._data).status_message
        : "No message";
      ...
      feedItem.addEventListener("click", () =>
        toggleComments(feedItem, item.id)
      );
...
(function () {
  ...
  function createCommentSection(postId, comments) {
    ...
    comments.forEach((comment) => {
      ...
      content.innerHTML = `
                    <a href="/profile/${
                      comment.profile_id
                    }" class="username-link" style="color: #6a1b9a;">${
        comment.profile_username
      }</a><br>
  ...
  function displayFeedData(feedData) {
    ...
    feedData.forEach((item) => {
      let statusMessage =
        "Feed object responsible either for new username, map or obtained avatar.";
      ...
      content.innerHTML = `
                    <div style="display: flex; align-items: center;">
                        <img src="${item.profile_images.medium}" alt="${
        ...
        item.id
                            ...
                            <button id="commentsButton-${
                              item.id
                            }" style="background: #eee; border: 1px solid #ddd; color: #666; padding: 5px 10px; border-radius: 5px; cursor: pointer; font-size: 12px; margin-top: 5px;">COMMENTS</button>
                            <div id="comments-${
                              item.id
                            }" style="display: none;"></div>
                            ${
                              item.can_delete
                                ? `<button style="background: #ffdddd; border: 1px solid #ddd; color: #ff0000; padding: 5px 10px; border-radius: 5px; cursor: pointer; font-size: 12px; margin-top: 5px;" onclick="deletePost(${item.id})">DELETE POST</button>`
                                : ""
                            }
                        </div>
                    </div>
...
(function () {
  ...
  function addExportButton() {
    ...
    if (targetElement && !document.querySelector(".export-btn")) {
      ...
      exportButton.title =
        "Export chat history to a text file. In case it does not work close and reopen the chat.";
      exportButton.style.width = "30px";
      exportButton.style.height = "30px";
      exportButton.style.background =
        "url(https://i.imgur.com/hG5QwIl.gif) center center / 16px 16px no-repeat";
  ...
  function replaceImageAndTooltip() {
    ...
    if (targetElement) {
      ...
      targetElement.setAttribute(
        "href",
        "https://www.github.com/ctaah/utilify"
      );
    }
  }
                    ...
                    ${games
                      .map(
                        ...
                        <div class="kogama-game">
                            <img src="${
                              game.image_medium
                            }" class="kogama-game-icon">
                            <div class="kogama-game-info">
                                <strong><a href="https://www.kogama.com/games/play/${
                                  game.id
                                }/" target="_blank" class="kogama-game-link">${
                          game.name
                        }</a></strong>
